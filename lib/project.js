...
module.exports.run = function( app_path, brk ) {
	...
	function step2( homey ){
		...
		pack( app_path, function( tmppath ){
			upload( tmppath, homey.ip_internal, token, brk, env, function( err, response ){
				...
				*/
				...
				var socket = socket_io_client( 'http://' + homey.ip_internal + '/realtime/manager/devkit/', {
					query: 'token=' + token
				});
				...
				socket.on('connect', function(){
					...
					// get the log buffer
					request.get({
						url: 'http://' + homey.ip_internal + '/api/manager/devkit/log/',
						headers: {
				...
				process.stdin.on('keypress', function (ch, key) {
				  if (key && key.ctrl && key.name == 'c') {
				    stop( homey.ip_internal, token, response.result.app_id, function(){
					    process.exit();
				    })
				  }
				});
...
function validate_app( app_path, appstore ) {
	...
	var app = new devkitlib.App( app_path );
